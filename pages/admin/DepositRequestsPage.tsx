...
const DepositRequestsPage: React.FC = () => {
  const { deposits, updateDepositStatus } = useData();
  const { user: adminUser } = useAuth();
  const [confirmAction, setConfirmAction] = useState<{ request: Deposit, status: 'Completed' | 'Rejected' } | null>(null);

  const handleActionClick = (request: Deposit, status: 'Completed' | 'Rejected') => {
    setConfirmAction({ request, status });
  };

  const handleConfirmAction = () => {
    if (confirmAction && adminUser) {
      updateDepositStatus(confirmAction.request.id, confirmAction.status, adminUser.username);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-brand-orange mb-8">Deposit Requests</h1>
            ...
            <tbody>
              {deposits.map(req => (
                <tr key={req.id} className="border-b border-brand-light-gray last:border-b-0">
                  ...
                  <td className="p-4 font-mono text-sm">{req.transactionId}</td>
                  <td className="p-4">
      ...
      <ConfirmationModal
        ...
        title={`${confirmAction?.status === 'Completed' ? 'Approve' : 'Reject'} Deposit`}
        message={
            <p>
                Are you sure you want to {confirmAction?.status === 'Completed' ? 'approve' : 'reject'} this deposit request of <strong>₹{confirmAction?.request.amount.toFixed(2)}</strong> for user <strong>{confirmAction?.request.username}</strong>?
            </p>
        }
